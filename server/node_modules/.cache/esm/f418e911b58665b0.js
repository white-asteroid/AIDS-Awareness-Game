let users,hashPassword,comparePassword,jwt;_b58‍.x([["Login",()=>Login],["signup",()=>signup]]);_b58‍.w("../models/users",[["default",["users"],function(v){users=v}]]);_b58‍.w("../helpers/auth",[["hashPassword",["hashPassword"],function(v){hashPassword=v}],["comparePassword",["comparePassword"],function(v){comparePassword=v}]]);_b58‍.w("jsonwebtoken",[["default",["jwt"],function(v){jwt=v}]]);
...
async function validateEmailAccessibility(email){
  ...
  validateEmailAccessibility(email.em).then(async function(valid) {
    if (valid) {
      ...
      _b58‍.g.console.log("YOUR email is L : ",Luser.email); 
      ...
      _b58‍.g.console.log(Luser);
     res.json({
      token,Luser,
     })
    }
    catch(err){
      _b58‍.g.console.log("LOGIN inside fun ",err);
    }
    }
  })
